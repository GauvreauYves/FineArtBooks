...
Scene-referred development preserves the integrity and authenticity of captured raw sensor data. A [correction LUT](https://3dlutcreator.com/) made from a ColorChecker shot in situ  ensures color and luminance accuracy (ΔE00 < 2) to even greater levels. I use mostly DCRaw_emu in combination with PureRaw4 from DXO, and 3DLut Creator. Recently, I created a custom DCP profile frm my cameras SSF that I use from Lightroom. This method benefit from the correction Lut strategy. The actual workflow is a bit complicated, as I use quite a few programs, but the results are worth it. It a bit kind of Ansel Adams of expose to obtain 0.1d (density) above base + fog. If you tried to approach this, you know what I mean.
